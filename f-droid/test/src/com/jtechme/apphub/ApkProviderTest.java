...
import com.jtechme.apphub.data.ApkProvider;
import com.jtechme.apphub.data.RepoProvider;
import com.jtechme.apphub.mock.MockApk;
import com.jtechme.apphub.mock.MockApp;
import com.jtechme.apphub.mock.MockRepo;
...
public class ApkProviderTest extends BaseApkProviderTest {
    ...
    /**
     * I want to test the protected {@link com.jtechme.apphub.data.ApkProvider#getContentUri(java.util.List)}
    ...
    public void testUris() {
        ...
        assertValidUri(ApkProvider.getAppUri("com.jtechme.apphub"));
        assertValidUri(ApkProvider.getContentUri(new MockApk("com.jtechme.apphub", 100)));
        assertValidUri(ApkProvider.getContentUri());
        assertValidUri(PublicApkProvider.getContentUri(apks));
        assertValidUri(ApkProvider.getContentUri("com.jtechme.apphub", 100));
        ...
        manyApks.add(new MockApk("com.jtechme.apphub.1", 1));
        manyApks.add(new MockApk("com.jtechme.apphub.2", 2));
        try {
    ...
    public void testAppApks() {
        for (int i = 1; i <= 10; i++) {
            TestUtils.insertApk(this, "com.jtechme.apphub", i);
        ...
        Cursor fdroidApks = getMockContentResolver().query(
                ApkProvider.getAppUri("com.jtechme.apphub"),
                getMinimalProjection(),
                null, null, null);
        assertResultCount(10, fdroidApks);
        assertBelongsToApp(fdroidApks, "com.jtechme.apphub");
        fdroidApks.close();
        ...
        assertBelongsToApp(all, "com.jtechme.apphub");
        all.close();
    }

    public void testInvalidUpdateUris() {
        Apk apk = new MockApk("com.jtechme.apphub", 10);

        List<Apk> apks = new ArrayList<>();
        apks.add(apk);

        assertCantUpdate(ApkProvider.getContentUri());
        assertCantUpdate(ApkProvider.getAppUri("com.jtechme.apphub"));
    ...
    public void testInvalidDeleteUris() {
        ...
        assertCantDelete(ApkProvider.getContentUri("com.jtechme.apphub", 10));
    ...
    public void testRepoApks() {
        ...
        // the apks from.
        for (int i = 1; i <= 5; i++) {
            insertApkForRepo("com.jtechme.apphub", i, REPO_KEEP);
            insertApkForRepo("com.example." + i, 1, REPO_DELETE);
        }
        for (int i = 6; i <= 10; i++) {
            insertApkForRepo("com.jtechme.apphub", i, REPO_DELETE);
    ...
    public void testInsert() {
        ...
        Apk apk = new MockApk("com.jtechme.apphub", 13);
        ...
        cursor.close();
        assertEquals("com.jtechme.apphub", toCheck.packageName);
    ...
    public void testInsertWithExtraFields() {
        ...
        for (String field : repoFields) {
            ...
            try {
                TestUtils.insertApk(this, "com.jtechme.apphub", 10, invalidRepo);
                fail();
            } catch (IllegalArgumentException e) {
            } catch (Exception e) {
                fail();
